...
export default function ControlCentrePage() {
	...
	useEffect(() => {
		...
		const marketIds = Array.from(new Set(alerts.map((a) => a.market.id)));

		const handleConnect = () => {
			socket.emit('SUBSCRIBE_TICKERS', marketIds);
		};
		...
		const handleTickerUpdate = (data: any) => {
			...
			setAlerts((prevAlerts) =>
				prevAlerts.map((alert) => {
					if (alert.market.id === symbol) {
						return {
							...alert,
							market: {
								...alert.market,
								yesPrice: lastPrice,
								noPrice: 10 - lastPrice,
							},
						};
					}
					return alert;
		...
		socket.on('MESSAGE', handleTickerUpdate);

		return () => {
			socket.emit('UNSUBSCRIBE_TICKERS', marketIds);
			socket.off('TICKER', handleTickerUpdate);
			socket.off('MESSAGE', handleTickerUpdate);
		...
		<div className="w-full bg-[#f4f4f5] dark:bg-[#090C1A] flex justify-center px-4 md:pt-16 pt-16 pb-16 transition-colors min-h-screen">
			<div className="w-full max-w-4xl flex flex-col gap-8">
				<div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-gray-200 dark:border-white/10">
					...
					<Link
						to="/events"
						className="hidden sm:flex items-center justify-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-xl bg-black dark:bg-white text-white dark:text-black"
					>
						Explore Markets
					</Link>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
					<div className="bg-white dark:bg-[#090C1A] p-4 rounded-xl border border-gray-200 dark:border-white/10 flex items-start justify-between group hover:border-gray-300 dark:hover:border-white/10 transition-colors">
						...
						<div className="p-3 text-black dark:text-white group-hover:scale-110 transition-transform">
					...
					<div className="bg-white dark:bg-[#090C1A] p-4 rounded-xl border border-gray-200 dark:border-white/10 flex items-start justify-between group hover:border-gray-300 dark:hover:border-white/10 transition-colors">
						<div className="space-y-1">
							...
							<p className="text-sm font-semibold text-gray-900 dark:text-white mt-2 flex items-center gap-1.5">
													...
													<div className="flex justify-between items-start gap-4">
														<div className="space-y-1.5 flex-1">
															<Link
																...
																className="text-base font-semibold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors line-clamp-2 pr-2"
															>
						...
						{activeTab === 'limits' && (
							<div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-5">
								<div className="bg-white dark:bg-[#090C1A] rounded-2xl border border-gray-200 dark:border-white/5 overflow-hidden">
									...
									<div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100 dark:divide-white/5 bg-gray-50/50 dark:bg-[#090C1A]">
								...
								<div className="bg-transparent border border-gray-200 dark:border-white/10 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
									<div>
									...
									<Link
										to="/settings"
										className="shrink-0 px-5 py-2.5 bg-white dark:bg-[#111111] text-blue-700 dark:text-blue-400 text-sm font-semibold rounded-xl border border-blue-200 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors"
									>
										Manage Settings
									</Link>
								</div>
